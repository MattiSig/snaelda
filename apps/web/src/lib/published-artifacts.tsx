...
export function buildPublishedArtifactBundle(
  ...
  const manifest: PublishedArtifactManifest = {
    ...
    pages: pages.map(({ pagePath, filePath, title, description, canonicalUrl }) => ({
      pagePath,
      filePath,
      title,
      description,
...
function buildSitemapXML(manifest: PublishedArtifactManifest) {
  return [
    ...
    ...manifest.pages.map((page) => `  <url><loc>${escapeXML(page.canonicalUrl)}</loc></url>`),
    '</urlset>',
    '',
  ].join('\n')
}
...
function buildRobotsTXT(manifest: PublishedArtifactManifest) {
  const sitemapPath = manifest.hostname ? '/sitemap.xml' : `/public/${manifest.siteSlug}/sitemap.xml`
  const sitemapURL = new URL(sitemapPath, manifest.pages[0]?.canonicalUrl ?? 'http://localhost:3000').toString()

  return ['User-agent: *', 'Allow: /', `Sitemap: ${sitemapURL}`, ''].join('\n')
}

function buildCanonicalURL(input: PublishedArtifactRenderInput, pagePath: string) {
